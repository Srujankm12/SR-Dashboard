...
<div class="flex justify-center items-center min-h-screen bg-gradient-to-br from-white via-white to-white">
  <div class="bg-white shadow-2xl rounded-lg p-8 max-w-md w-full border-t-4 border-black">
    ...
    <form on:submit|preventDefault={handleRegister} class="space-y-6">
      ...
      <button
        type="submit"
        class="w-full bg-black text-white font-semibold py-3 px-4 rounded-lg shadow-lg  focus:outline-none focus:ring transition"
        disabled={loading}
      >
    ...
    <div class="mt-8 text-center text-sm text-gray-500">
      <p>Already have an account? <a href="/login" class="text-black font-medium hover:underline">Login</a></p>
    </div>
  </div>
</div>
